...
func setupTestStore(t *testing.T) (*FirestoreStore, func()) {
	t.Helper()
	...
	resourcesColl := "resources_test_" + suffix
	countersColl := "counters_test_" + suffix
	...
	cleanup := func() {
		deleteCollection(ctx, fsClient, resourcesColl)
		deleteCollection(ctx, fsClient, countersColl)
		fsClient.Close()
	}
...
func deleteCollection(ctx context.Context, client *firestore.Client, collName string) {
...
func TestFirestoreStore_Create(t *testing.T) {
...
func TestFirestoreStore_ResourceVersionIncrement(t *testing.T) {
	store, cleanup := setupTestStore(t)
	defer cleanup()
